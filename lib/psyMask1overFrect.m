function [msk1oF,mskNoise,mskHole] = psyMask1overFrect(PszXY,diameterXY,CctrXY,bPLOT)

% function [msk1oF mskNoise mskHole] = psyMask1overF(PszXY,diameterK,CctrXY,bPLOT,bSQR)
%
...
% diameterK: fraction of min screen dimension to use for diameter of mask
...
% mskNoise = (mskNoise - min(mskNoise(:)))./max(mskNoise(:) - min(mskNoise(:)));

% MAKE CIRCLE FOR ALPHA CHANNEL
mskHole = ~rectImage(PszXY,0.5.*diameterXY.*PszXY,CctrXY);
...
if bPLOT
    ...
    Fig.format([],[],'1/F Noise'); caxis([0 1])

    % PLOT MASK
    subplot(1,3,2);
    imagesc(mskHole.^expnt); axis image;  axis xy
    Fig.format([],[],['Mask: x_0=[' num2str(CctrXY(1)) ' ' num2str(CctrXY(2)) ']']); caxis([0 1])
